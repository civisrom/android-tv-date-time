...
- `adb/` — собственный каталог `adb` (внутри него `.android`) для беспроводной отладки
  Android 11+: там лежат ключ и список спаренных устройств. Программа работает на
  собственном порту ADB-сервера, поэтому не мешает вашему `adb` и Android Studio.
  При первом запуске существующие ключи из `~/.android` копируются сюда, чтобы уже
  спаренные устройства продолжили работать.
