...
1.  Подключите ваш Android TV (или Nvidia Shield) и компьютер к одной локальной сети.
2.  Узнайте IP-адрес вашего Android TV в разделе **"Настройки > Сеть и Интернет"**.
3.  Запустите программу `AndroidTVTimeFixer.exe` на компьютере.
4.  Следуйте инструкциям в программе для подключения к вашему Android TV и настройки NTP-сервера.
