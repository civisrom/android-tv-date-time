...
- **adb 37.0.1 берёт свой каталог ТОЛЬКО из `HOME`/`USERPROFILE`.** Проверено
  на настоящем бинарнике: `ANDROID_USER_HOME` и `ANDROID_SDK_HOME` он
  игнорирует и всё равно пишет в `$HOME/.android`. Поэтому изоляция делается
  подменой `HOME` в `env=` дочернего процесса (никогда в `os.environ` — иначе
  поедут `Path.home()` и platformdirs самой программы), а рабочий каталог adb
  получается `<adb_home>/.android`.
