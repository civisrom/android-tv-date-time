...
internal fun SetupScreen(state: AppState, actions: AppActions, onBack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().safeDrawingPadding().verticalScroll(scroll).padding(horizontal = 48.dp, vertical = 27.dp)
        ...
        FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(shape = MaterialTheme.shapes.medium, onClick = { if (step < 2) step++ else onBack() },
                modifier = Modifier.focusRequester(nextFocus).focusProperties { canFocus = true }.testTag("setup-next")) {
                Text(stringResource(if (step < 2) R.string.setup_next else R.string.setup_finish))
            }
            TextButton(shape = MaterialTheme.shapes.medium, onClick = { if (step > 0) step-- else onBack() }, modifier = Modifier.testTag("setup-back")) {
        ...
        when (step) {
            0 -> {
                ...
                Button(shape = MaterialTheme.shapes.medium, onClick = { actions.openSetupSettings(Settings.ACTION_DEVICE_INFO_SETTINGS) },
                    modifier = Modifier.testTag("setup-about")) { Text(stringResource(R.string.setup_about)) }
                Button(shape = MaterialTheme.shapes.medium, onClick = { actions.openSetupSettings(Settings.ACTION_APPLICATION_DEVELOPMENT_SETTINGS) },
            ...
            1 -> {
                ...
                Button(shape = MaterialTheme.shapes.medium, onClick = { actions.openSetupSettings(Settings.ACTION_DATE_SETTINGS) }) {
